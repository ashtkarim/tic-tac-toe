...
const icon = <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="size-6">
...
export default function AsideNav({ children }) {
	/* Still in progress to make it mobile friendly :) */
	...
	return (
		<>
			<div>
				{/* Currently this layout works perfectly in the desktop*/}
				<div className={`hidden md:fixed md:inset-y-0 md:flex md:w-${expand ? '64' : '14'} w-${expand ? '64' : '14'}`}>
					...
					<button className='absolute bg-primary text-white text-bold top-1/2 right-[-20px] rounded w-8 h-8'
						onClick={() => setExpand(!expand)}>{expand ? '<<' : '>>'}</button>
				</div>
					...
					<main className="flex-1 min-h-screen bg-primaryLight">
						<div className="py-6">
							...
							<div className="mx-auto max-w-7xl px-4 sm:px-6 md:px-8">
								{/* This is your place where you can play */}
								<div className="py-4">
									{children}
								</div>
								{/* /End of place area */}
